...
# Usage: tools/decomp/run_batch.sh [count]
set -euo pipefail
...
if [ ! -d "$MIZ" ]; then
  echo "error: Mizuchi not installed — run build_tools.sh" >&2
  exit 1
fi
...
python3 tools/decomp/make_prompts.py -n "$COUNT"
...
echo "==> Done. Summary: docs/decomp-status.md"
