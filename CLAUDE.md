...
# 2. Match + integrate + commit (10 functions) — no-op once all readable-Thumb
# functions are asm-matched (they are, as of 2026-09-19: 399/399). Cheap to
# run each session in case new functions get peeled; do not treat "0
# integrated" as a failure.
...
# 4. Phase 3b (the real remaining work): mechanical batch, then hand-convert
# from the categorized plan — see docs/decomp-semantic-plan.md
tools/decomp/c_convert_batch.sh 30
python3 tools/decomp/classify_semantic_targets.py            # pick a bucket
python3 tools/decomp/classify_semantic_targets.py --symbols   # unblock first
